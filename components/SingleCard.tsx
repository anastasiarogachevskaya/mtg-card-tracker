...
import {
	...
	CMCProps,
	SingleCardProps,
} from '../types/Card/SingleCardProps';
...
const Cost = styled.abbr<AbbrProps>`
	${({ cost }) =>
		cost &&
		`background: url('/img/manacost/${cost}.svg') no-repeat center center`};
	padding: 15px;
	margin: 5px;
...
const Mana = styled.abbr<AbbrProps>`
	${({ colorName }) =>
		colorName &&
		`background: url('/img/color/${colorName}.svg') no-repeat center center`};
	padding: 15px;
	margin: 5px;
	vertical-align: bottom;
`;
...
const SingleCard = ({ data }: { data: SingleCardProps }) => {
	...
	const priceGrid = Object.keys(prices).map((key) => {
		...
		return (
			<React.Fragment key={price + key}>
				<StyledB uppercase>{currency}</StyledB>
	...
	return (
		<Wrapper>
			<ImageWrapper>
				{imageSRC && <StyledImage src={imageSRC} alt={name} />}
			</ImageWrapper>
			<InfoWrapper>
				<Title>
					{name}
					<ManaInfo>
						{cmc.map((element) => {
							if (element.type === 'cost') {
								return <Cost key={element.title} cost={element.title} />;
							} else {
								return <Mana key={element.title} colorName={element.title} />;
							}
						})}
					</ManaInfo>
				</Title>
				...
				<Grid>
					<StyledB>Expansion: </StyledB>
					<StyledText>{set_name}</StyledText>
					<StyledB>Rarity: </StyledB>
					<StyledText>{rarity}</StyledText>
					<StyledB>Released: </StyledB>
					<StyledText>{released_at}</StyledText>
					<StyledB>Card Number: </StyledB>
					<StyledText>{collector_number}</StyledText>
					<StyledB>Illustrated by: </StyledB>
					<Link
						href={`/search?q=${encodeURIComponent(`a:"${artist}"&unique=art`)}`}
					>

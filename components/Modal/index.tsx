...
import {
  StyledDarkBg, StyledCentered, StyledModal, StyledModalHeader,
  StyledHeading, StyledCloseButton, StyledContent, StyledActions,
  StyledActionsContainer, StyledAddButton, StyledCancelButton,
  StyledLabel, StyledCheckboxWrapper
} from "./ModalParts";
...
const Modal = ({ setIsOpen, decks, cardInfo }: ModalProps) => {
  ...
  const handleCheckboxChange = (event: { target: { checked: boolean | ((prevState: boolean) => boolean); value: React.SetStateAction<string>; }; }) => {
    setChecked(event.target.checked);
    if (event.target.checked === true) {
      if (decksToUpdate.find((element: React.SetStateAction<string>) => element === event.target.value)) {
        return;
      } else {
        setDecksToUpdate((prevArray: any) => [...prevArray, event.target.value])
      }
    } else {
